...
class MyWindow(QWidget): 
	...
	def btn3_switch(self):
		if self.btn3.isEnabled() == True and self.btn4.isChecked():
			self.btn3.setEnabled(False)
			self.btn3.setToolTip('First click \'Compute Contact Times\' to enable button.\nMake sure sun is in focus.\'Camera Preview\' must be toggled off to capture eclipse')
		else:
	...
	def compute_contacts(self):
		...
		#ENTER DATES AND TIMES HERE TO SIMULATE ECLIPSE
		self.tc2 =  dt.datetime.strptime('2017-08-11 21:41:30', "%Y-%m-%d %H:%M:%S") 
		self.tc3 = dt.datetime.strptime('2017-08-11 21:42:00', "%Y-%m-%d %H:%M:%S") 
